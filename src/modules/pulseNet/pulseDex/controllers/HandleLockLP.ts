...
class HandleLockLP {
    public async index() {
        ...
        const lockedTime = new Date(new Date().getTime() - 6 * 60 * 60 * 1000).valueOf(); // 6h
            ...
            for (let lp of list.lp_address) {
                ...
                if (new Date(list.created).valueOf() <=  lockedTime || list?.counter >= 3) {
                    for (const lpLocker of listLPLocker) {
                        const tx = await Helper.AddLockerLP(config.rpc, config.chainId, list?.token_address, lp, config.privateKeyRouter, routerAbi, config.lockerAddress,  [lpLocker.userAddress, lpLocker.start, 36500 * (24 * 60 * 60)])
                        if (tx.status) {
